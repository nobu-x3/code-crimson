...
begin_frame :: proc(ctx: ^rd.Context, delta_time: f64) {
    ...
    vk.ResetCommandBuffer(ctx.command_buffers[ctx.curr_frame], {})
	rd.command_buffer_begin(ctx.command_buffers[ctx.curr_frame], {})
	...
	vk.CmdSetViewport(ctx.command_buffers[ctx.curr_frame], 0, 1, &viewport)
	vk.CmdSetScissor(ctx.command_buffers[ctx.curr_frame], 0, 1, &scissor)
	...
	rd.render_pass_begin(
		&ctx.main_render_pass,
		ctx.command_buffers[ctx.curr_frame],
...
end_frame :: proc(ctx: ^rd.Context) {
	...
	rd.command_buffer_end(ctx.command_buffers[ctx.curr_frame])
	submit_info: vk.SubmitInfo
	submit_info.sType = .SUBMIT_INFO
	submit_info.commandBufferCount = 1
	submit_info.pCommandBuffers = &ctx.command_buffers[ctx.curr_frame]
